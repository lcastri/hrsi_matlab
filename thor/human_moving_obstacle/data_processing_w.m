    ...
    for i = 1 : length(A)
        if ~isempty(A{i}.goals)
            ...
            if ~isnan(A{i}.x(t))
                ...
                % Bearing g
                A{i}.theta_a(t, A{i}.goals_seq(t)) = atan2(A{i}.goals{A{i}.g}.y - A{i}.y(t), A{i}.goals{A{i}.g}.x - A{i}.x(t));
                A{i}.bearing(t) = atan2(A{i}.y(t) - A{i}.goals{A{i}.g}.y, A{i}.x(t) - A{i}.goals{A{i}.g}.x) + pi - A{i}.theta(t);
                ...
                [ciao, A{i}.dobs(t)] = measure_obs(A, i, G, eta_0, t, 0);
            end
        end
    end
end
...
for i = 1 : length(A)
    A{i}.bearing(:) = medfilt1(A{i}.bearing(:), 25);
    A{i}.bearing = wrapTo2Pi(A{i}.bearing);
end
